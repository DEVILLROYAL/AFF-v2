         ...
         <div className="overflow-auto overflow-y-hidden h-[50vw] w-full whitespace-nowrap">
          { slider.map((item, index)=>{return(<div className="inline-block relative h-full w-full" key={item.id} >
             <div className="w-full h-full bg-cover bg-no-repeat" style={{backgroundImage:`url(${item.img})`}}>
              ...
              <div className="p-1 md:p-2 md:m-2 xl:p-2 xl:rounded m-1 absolute xl:m-5 text-xs xl:text-xl text-white top-0 left-0 bg-gray-600" >
                <p>Spotlight {item.rank}</p>
              </div>
              <div className="flex flex-row-reverse  text-white p-1 md:p-2 md:gap-3 xl:gap-5 xl:p-5 gap-2 text-xs xl:text-xl absolute top-0 right-0">
              <div className="w-max p-1 md:p-2 xl:rounded xl:p-2 bg-black" >
                <p>
                   {item.quality}
                </p>
              </div>
              <div className="w-max p-1 xl:rounded md:p-2 xl:p-2 bg-blue-600" >
                <p>
                   {item.category}
                </p>
              </div>
