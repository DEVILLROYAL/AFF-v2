...
export default async function HindiAnime() {
  const ul = "https://youtubeapi-tmc9.onrender.com/api/home";
  const data = await fetch(ul);
  const res = await data.json();

  // Check if your API has a "hindi" field or adjust accordingly
  const hindi = res?.hindi || res || [];
  ...
  return (
    <div className="bg-black text-white p-2 xl:p-5 md:p-5">
      ...
      <div className="overflow-auto scroll-smooth inset-shadow-black overflow-y-hidden w-full h-[250px] md:h-[250px] whitespace-nowrap">
        {Array.isArray(hindi) && hindi.length > 0 ? (
          hindi.map((item, index) => (
            <YtAnimeCard
              key={index}
              id={item.playlistId}
              image={item.image}
              name={item.engTitle ?? item.animeTitle}
            />
          ))
        ) : (
          <p className="text-center text-gray-400">No Hindi anime found.</p>
        )}
      </div>
    </div>
  );
